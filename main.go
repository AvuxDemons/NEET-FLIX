	...
	if model == 1 {
		...
		for x != "t" {
			...
			if selector == 1 {
					...
					for date >31 && date <1{
						fmt.Scan(&date)
						fmt.Println("| -> gak masuk akal, baleni <-")
					}
					fmt.Print("| Bulan   : ")
					...
					fmt.Print("| Tahun   : ")
					for year <(2023-100) && year >(2023-5){
					fmt.Scan(&year)
					if year <(2023-100){
						fmt.Println("| -> Gak capek hidup ? <-")
					} else if year >(2023-5){
						fmt.Println("| -> Ngapain disini cil <-")
					}
					}
					scanner.Scan()
			} else {
		...
		if selector == 1 {
				fmt.Println("USER <-")
				// jika data ada prosesnya lanjut, kalo tidak ada notif data tidak ada
				fmt.Print("| Username   : ")
				scanner.Scan()
				a = scanner.Text()
				fmt.Print("| Email      : ")
